    ...
    async function send(envelope$1, isRetry = false) {
      ...
      try {
        if (options.shouldSend) {
          const decision = options.shouldSend(envelope$1);
          const shouldSend = is.isThenable(decision) ? await decision : decision;
          if (shouldSend === false) {
        ...
        const decision = shouldQueue(envelope$1, e, retryDelay);
        if (is.isThenable(decision) ? await decision : decision) {
          if (isRetry) {
