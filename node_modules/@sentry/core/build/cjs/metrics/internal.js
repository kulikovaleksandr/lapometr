...
function _INTERNAL_captureMetric(beforeMetric, options) {
  ...
  const processedMetric = beforeSendMetric ? safeCallback.safeCallback(
    debugBuild.DEBUG_BUILD ? "The `beforeSendMetric` callback threw an error, dropping the metric:" : "",
    () => beforeSendMetric(enrichedMetric),
    () => safeCallback.CALLBACK_ERROR
  ) : enrichedMetric;
  if (processedMetric === safeCallback.CALLBACK_ERROR) {
