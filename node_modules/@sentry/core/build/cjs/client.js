...
const warnAboutIgnoredTransactionOptions = require('./utils/warnAboutIgnoredTransactionOptions.js');
...
class Client {
  ...
  constructor(options) {
    ...
    const { beforeSendSpan: beforeSendSpan$1, traceLifecycle } = this._options;
    if (debugBuild.DEBUG_BUILD && beforeSendSpan$1 && beforeSendSpan.isStaticBeforeSendSpanCallback(beforeSendSpan$1) !== (traceLifecycle === "static")) {
      debugLogger.consoleSandbox(() => {
        console.warn(
          `Ignoring \`beforeSendSpan\`: ${traceLifecycle === "static" ? "wrap it with" : "remove"} \`Sentry.withStaticSpan\` to use it with \`traceLifecycle: "${traceLifecycle}"\`.`
        );
      });
    }
    if (this._dsn) {
  ...
  captureEvent(event, hint, currentScope) {
    ...
    const dataCategory = envelope.getDataCategoryByType(event.type);
    this._process(
  ...
  init() {
    ...
    this._options.integrations.some(({ name }) => name.startsWith("Spotlight"))) {
      ...
      warnAboutIgnoredTransactionOptions.maybeWarnAboutIgnoredTransactionOptions(this._options);
    }
  }
  /**
  ...
  sendEvent(event, hint = {}) {
    ...
    let env = envelope$1.createEventEnvelope(event, this._dsn, this._options._metadata, this._options.tunnel);
    for (const attachment of hint.attachments || []) {
      env = envelope.addItemToEnvelope(env, envelope.createAttachmentEnvelopeItem(attachment));
    }
    if (genAiSpanItem) {
      env = envelope.addItemToEnvelope(env, genAiSpanItem);
    }
  ...
  sendSession(session) {
    ...
    const env = envelope$1.createSessionEnvelope(session, this._dsn, this._options._metadata, this._options.tunnel);
  ...
  _updateSessionFromEvent(session$1, event) {
    ...
    const shouldUpdateAndSend = sessionNonTerminal && session$1.errors === 0 || sessionNonTerminal && unhandled;
    if (shouldUpdateAndSend) {
      session.updateSession(session$1, {
        ...unhandled && { status: this._unhandledSessionStatus },
        errors: session$1.errors || Number(errored || unhandled)
      });
  ...
  _processEvent(event, hint, currentScope, isolationScope) {
    ...
    let beforeSendDropReason = "before_send";
...
function processBeforeSend(client, options, event, hint, onCallbackError) {
  const {
    beforeSend,
    ignoreSpans,
  ...
  const beforeSendSpan$1 = beforeSendSpan.isStaticBeforeSendSpanCallback(options.beforeSendSpan) && options.beforeSendSpan;
  let processedEvent = event;
  if (isErrorEvent(processedEvent) && beforeSend) {
    const errorEvent = processedEvent;
    return safeCallback.safeCallback(
      debugBuild.DEBUG_BUILD ? "The `beforeSend` callback threw an error, dropping the event:" : "",
  ...
  if (isTransactionEvent(processedEvent)) {
    if (beforeSendSpan$1 || ignoreSpans) {
      ...
      if (beforeSendSpan$1) {
        const processedRootSpanJson = beforeSendSpan.applyBeforeSendSpanCallback(rootSpanJson, beforeSendSpan$1);
      ...
      if (processedEvent.spans) {
        ...
        for (const span of initialSpans) {
          ...
          if (beforeSendSpan$1) {
            processedSpans.push(beforeSendSpan.applyBeforeSendSpanCallback(span, beforeSendSpan$1));
          } else {
    ...
    if (beforeSendTransaction) {
      ...
      return safeCallback.safeCallback(
        debugBuild.DEBUG_BUILD ? "The `beforeSendTransaction` callback threw an error, dropping the event:" : "",
        () => beforeSendTransaction(processedEvent, hint),
        () => {
          onCallbackError();
          return null;
        }
