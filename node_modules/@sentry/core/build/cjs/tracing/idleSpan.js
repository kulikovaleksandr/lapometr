...
const browserSpanApi = require('./browserSpanApi.js');
...
function startIdleSpan(startSpanOptions, options = {}) {
  ...
  browserSpanApi._INTERNAL_ensureBrowserSpanStreaming(client);
  ...
  const span = trace.startInactiveSpan(startSpanOptions);
  ...
  span.end = new Proxy(span.end, {
    apply(target, thisArg, args) {
      ...
      const latestSpanEndTimestamp = spans?.reduce((acc, current) => {
        const currentSpanJson = spanUtils.spanToStaticSpanJSON(current);
  ...
  function onIdleSpanEnded(endTimestamp) {
    ...
    childSpans.forEach((childSpan) => {
      ...
      const childSpanJSON = spanUtils.spanToStaticSpanJSON(childSpan);
  ...
  _cleanupHooks.push(
    client.on("spanStart", (startedSpan) => {
      if (_finished || startedSpan === span || !!spanUtils.spanToJSON(startedSpan).end_timestamp || startedSpan instanceof sentrySpan.SentrySpan && startedSpan.isStandaloneSpan()) {
        return;
      }
