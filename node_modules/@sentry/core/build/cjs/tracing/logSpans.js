...
function logSpanStart(span) {
  ...
  } = spanUtils.spanToStaticSpanJSON(span);
  ...
  if (!isRootSpan) {
    const { op: op2, description: description2 } = spanUtils.spanToStaticSpanJSON(rootSpan);
...
function logSpanEnd(span) {
  if (!debugBuild.DEBUG_BUILD) return;
  const { description = "< unknown name >", op = "< unknown op >" } = spanUtils.spanToStaticSpanJSON(span);
  const { spanId } = span.spanContext();
  const rootSpan = spanUtils.getRootSpan(span);
  const isRootSpan = rootSpan === span;
