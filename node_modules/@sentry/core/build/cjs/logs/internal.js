...
function _INTERNAL_captureLog(beforeLog, currentScope = currentScopes.getCurrentScope(), captureSerializedLog = _INTERNAL_captureSerializedLog) {
  ...
  const span = spanUtils.getActiveSpan(currentScope);
  setLogAttribute(processedLogAttributes, "sentry.trace.parent_span_id", span?.spanContext().spanId);
  const processedLog = { ...beforeLog, attributes: processedLogAttributes };
  client.emit("beforeCaptureLog", processedLog);
  const log = beforeSendLog ? safeCallback.safeCallback(
    debugBuild.DEBUG_BUILD ? "The `beforeSendLog` callback threw an error, dropping the log:" : "",
    // We need to wrap this in `consoleSandbox` to avoid recursive calls to `beforeSendLog`
    () => debugLogger.consoleSandbox(() => beforeSendLog(processedLog)),
    () => safeCallback.CALLBACK_ERROR
  ) : processedLog;
  if (log === safeCallback.CALLBACK_ERROR) {
