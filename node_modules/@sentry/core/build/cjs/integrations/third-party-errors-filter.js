...
const thirdPartyErrorFilterIntegration = integration.defineIntegration((options) => {
  ...
  return {
    ...
    preprocessEvent(event) {
      if (ignoreSentryInternalFrames && (worldwide.GLOBAL_OBJ._sentryWrappedDepth ?? 0) > 0) {
    ...
    processEvent(event) {
      const insideSentryWrapped = ignoreSentryInternalFrames ? event.sdkProcessingMetadata?.insideSentryWrapped === true && event.exception?.values?.length === 1 : false;
