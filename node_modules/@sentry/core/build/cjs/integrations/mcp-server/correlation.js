...
function completeSpanWithResults(transport, requestId, result, error) {
  ...
  if (spanData) {
    ...
    if (responseSessionData.protocolVersion) {
      responseAttributes[attributes$1.MCP_PROTOCOL_VERSION_ATTRIBUTE] = responseSessionData.protocolVersion;
    }
    if (Object.keys(responseAttributes).length > 0) {
      span.setAttributes(responseAttributes);
    }
    if (error) {
      span.setAttributes({ [attributes.RPC_RESPONSE_STATUS_CODE]: String(error.code) });
      if (errorCapture.isJsonRpcServerError(error.code)) {
        span.setAttributes({ [attributes.ERROR_TYPE]: String(error.code) });
        span.setStatus({ code: spanstatus.SPAN_STATUS_ERROR, message: error.message });
      }
    } else if (method === "tools/call") {
      const toolAttributes = resultExtraction.extractToolResultAttributes(result, spanData.capturePolicy.recordOutputs);
      span.setAttributes(toolAttributes);
      if (toolAttributes[attributes$1.MCP_TOOL_RESULT_IS_ERROR_ATTRIBUTE] === true) {
        span.setAttributes({ [attributes.ERROR_TYPE]: "tool_error" });
