...
const errorSpanAttribution = require('./errorSpanAttribution.js');
...
function prepareEvent(options, event, hint, scope, client, isolationScope) {
  ...
  if (hint.mechanism) {
    misc.addExceptionMechanismToCapturedException(prepared, hint.mechanism);
  }
  ...
  const span = spanUtils.getActiveSpan(finalScope);
  if (span) {
    scopeData.applySpanToEvent(prepared, span);
  }
  errorSpanAttribution.applyEscapedErrorSpanToEvent(prepared, hint, finalScope);
  ...
  const result = isInternalException ? syncpromise.resolvedSyncPromise(prepared) : eventProcessors.notifyEventProcessors(eventProcessors$1, prepared, hint, 0, (reason) => {
    if (!client) {
      return;
    }
    client.recordDroppedEvent(reason, envelope.getDataCategoryByType(event.type));
