...
function sendIndexPath(pathBuilder, pathname, basename, stripBasename) {
  const reconstructedPath = pathBuilder && pathBuilder.length > 0 ? pathBuilder : stripBasename ? routeManifest.stripBasenameFromPathname(pathname, basename) : pathname;
  let formattedPath = (
...
function rebuildRoutePathFromAllRoutes(allRoutes, location, matchRoutes) {
  const matchedRoutes = matchRoutes(allRoutes, location);
...
function reconstructNameFromDescendantParent(location, allRoutes, currentName, matchRoutes) {
  const descendantParents = allRoutes.filter(routeIsDescendant);
  if (!descendantParents.length) {
    return void 0;
  }
  const matchedParents = matchRoutes(descendantParents, location);
...
function locationIsInsideDescendantRoute(location, routes, matchRoutes) {
...
function getFallbackTransactionName(location, basename, stripBasename) {
  return stripBasename ? routeManifest.stripBasenameFromPathname(location.pathname, basename) : location.pathname || "";
}
function getNormalizedName(routes, location, branches, basename = "", stripBasename = false) {
  if (!routes || routes.length === 0) {
    return [stripBasename ? routeManifest.stripBasenameFromPathname(location.pathname, basename) : location.pathname, "url"];
  }
  if (!branches) {
...
function resolveRouteNameAndSource(location, routes, allRoutes, branches, config) {
...
function getActiveRootSpan() {
  ...
  const rootSpan = span ? core.getRootSpan(span) : void 0;
  if (!rootSpan) {
    return void 0;
  }
