...
function _fetchResponseHandler(options, requestInfo, response, requestInit, error) {
  if (_shouldCaptureResponse(options, response.status, response.url)) {
    ...
    if (dc.requestHeaders !== false) {
      requestHeaders = core._INTERNAL_filterKeyValueData(_extractFetchHeaders(request.headers), dc.requestHeaders);
    }
    if (dc.responseHeaders !== false) {
      responseHeaders = core._INTERNAL_filterKeyValueData(_extractFetchHeaders(response.headers), dc.responseHeaders);
    }
    if (dc.cookies !== false) {
      const reqCookieStr = request.headers.get("Cookie") || void 0;
      if (reqCookieStr) {
        requestCookies = core._INTERNAL_filterCookies(reqCookieStr, dc.cookies, "cookie");
      }
      const resCookieStr = response.headers.get("Set-Cookie") || void 0;
      if (resCookieStr) {
        responseCookies = core._INTERNAL_filterCookies(resCookieStr, dc.cookies, "set-cookie");
      }
    }
  ...
  if (_shouldCaptureResponse(options, xhr.status, xhr.responseURL)) {
    ...
    if (dc.cookies !== false) {
      try {
        ...
        if (cookieString) {
          responseCookies = core._INTERNAL_filterCookies(cookieString, dc.cookies, "set-cookie");
        }
      } catch {
      }
    }
    if (dc.responseHeaders !== false) {
      try {
        responseHeaders = core._INTERNAL_filterKeyValueData(_getXHRResponseHeaders(xhr), dc.responseHeaders);
      } catch {
      }
    }
    if (dc.requestHeaders !== false) {
      requestHeaders = core._INTERNAL_filterKeyValueData(headers, dc.requestHeaders);
    }
    const event = _createEvent({
      url: xhr.responseURL,
      method,
